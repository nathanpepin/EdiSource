...
public sealed partial class Element(IEnumerable<string>? values = null) : IList<string>
{
    ...
    public static implicit operator Element(string[] values) => new(values);

    public static implicit operator Element(string value) => new([value]);

    public static implicit operator string(Element element) => element._compositeElements[0];

    public static implicit operator string[](Element element) => element._compositeElements.ToArray();

    public static Element[] FromString(string segmentText, Separators separators = default)
    {
        return segmentText
            .Trim()
