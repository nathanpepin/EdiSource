...
/// <summary>
///     Validates that a composite element value is a valid integer.
/// </summary>
    ...
    public IEnumerable<ValidationMessage> Validate(IEdi element)
    {
        ...
        if (segment.GetCompositeElementOrNull(dataElement, compositeElement) is { } value
            ...
            yield return ValidationFactory.Create(
                segment,
                validationSeverity,
                $"Data element {dataElement} in composite element {compositeElement} should be an integer but is not",
                dataElement);
    }
}