...
public static class EdiCommon
{
    ...
    private static void ValidateIsa(StreamReader stream)
    {
        ...
        Span<char> header = stackalloc char[3];
        var charsRead = stream.Read(header);

        if (charsRead < 3 || header[0] != 'I' || header[1] != 'S' || header[2] != 'A')
            throw new InvalidISAException("Expected ISA segment identifier at start of stream.");
    ...
    {
        ArgumentNullException.ThrowIfNull(ediText);
