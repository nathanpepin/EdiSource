...
{
    public string? StringElement { get => GetCompositeElementOrNull(1); set => value?.Do(x => SetCompositeElement(x, 1)); }
    public int? IntElement { get => this.GetInt(2); set => this.SetInt(value, 2); }
    public decimal? DecimalElement { get => this.GetDecimal(3); set => this.SetDecimal(value, 3); }
    public DateTime? DateElement { get => this.GetDate(4); set => this.SetDate(value, 4); }
    public TimeOnly? TimeElement { get => this.GetTimeOnly(5); set => this.SetTimeOnly(value, 5); }
    public bool? BoolElement { get => this.GetBool(6, "Y"); set => this.SetBool(value, "Y", "N", 6); }
}
```
...
### Type Mapping:
- **String/AN**: `string?` via `GetCompositeElementOrNull` / `SetCompositeElement`
- **Numeric/N**: `int?` via `GetInt` / `SetInt`, `decimal?` via `GetDecimal` / `SetDecimal`
- **Date**: `DateTime?` via `GetDate` / `SetDate` (format defaults to `"yyyyMMdd"`), or `DateOnly?` via `GetDateOnly` / `SetDateOnly`
- **Time**: `TimeOnly?` via `GetTimeOnly` / `SetTimeOnly` (format defaults to `"HHmm"`)
- **Boolean**: `bool?` via `GetBool(element, trueValue)` / `SetBool(value, trueValue, falseValue, element)` for Y/N indicators
- **Code Values**: Consider enums via `GetEnum<T>` / `SetEnum<T>` for better type safety
...
```csharp
[Fact]
public async Task CanParseBasic{TransactionSet}()
{
    InterchangeEnvelope.TransactionSetDefinitions.Add(
        _{TS}_{Ver}_{TransactionName}.Definition);
    ...
    var result = envelope.FunctionalGroups[0].TransactionSets[0] as _{TS}_{Ver}_{TransactionName};
